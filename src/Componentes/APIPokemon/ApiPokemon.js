...
const ApiPokemon = () => {
    ...
    useEffect(()=>{
        if (clicks !== ''){
            ...
            axios.get("https://pokeapi.co/api/v2/pokemon")  //atravez de get hacemos un llamado, un request
                .then(response => response.data)
                .then(data => {
                    let nombre = data.results; //depende a que informacion quiere entrar con el data.results o 
                    setPokemon(nombre);
                console.log("nombre")
                })
        }
